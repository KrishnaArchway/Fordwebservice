...
public class VinController {
  ...
  @PostMapping({"/vin/validatevin"})
  public ResponseEntity<VinResponseEntity> validateVin(@RequestBody VinRequestEntity vinRequestEntity, @RequestHeader("Authorization") String requestTokenHeader) throws UnirestException, JsonProcessingException {
    String methodName = "VinController.java validateVin(VinRequestEntity)";
    ...
    try {
      if (this.vinImplHelper.dokeyValidation(key)) {
        if (this.vinImplHelper.doParametersValidation(vin, pacode)) {
          ...
          vre.setVin(vin);
          String value = this.dmlManager.getAuthVinRegistration4WebService(vin, pacode);
